...
import { ExternalLink, Info, ChevronDown, Star, Video, Bot, Camera, Crown, Zap, Heart, Flame, Gift } from 'lucide-react';
...
export default function TopListPage() {
  ...
  const renderLoadingSkeleton = () => (
    [...Array(3)].map((_, index) => (
      <Card key={index} className="p-4 animate-pulse flex justify-between items-center">
  ...
  useEffect(() => {
    if (categories.length > 0) {
      const randomizedCategories = categories.map(category => ({
        ...category,
        randomIcon: category.slug2 === 'all' ? 'Star' : (category.icon_name as keyof typeof iconMap || iconList[Math.floor(Math.random() * iconList.length)] as keyof typeof iconMap),
      }));
  ...
  const handleCardClick = (e: React.MouseEvent<HTMLDivElement>, siteId: string) => {
  ...
  const activeCategDetails: CategoryWithIcon = useMemo(() => {
    return activeCategory === 'all'
      ...
      : categoriesWithIcons.find(c => c.slug2 === activeCategory) || categoriesWithIcons.find(c => c.slug2 === 'all') || {
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Discover the best sites categorized by users. Browse, search and explore!" />
      </Head>
      <div className="min-h-screen w-full overflow-hidden bg-gradient-to-b from-red-50 via-background to-background/95 dark:from-red-950/20 dark:via-background dark:to-background/95">
        <div className="w-full px-2 sm:container sm:mx-auto sm:px-6 pt-16 sm:pt-24 pb-8 sm:pb-16">
          {isCategoriesLoading ? (
            ...
            <>
              <div className="max-w-xl mx-auto mb-8 mt-6 sm:mt-0">
                <Select value={activeCategory} onValueChange={setActiveCategory}>
                    ...
                    <SelectValue>
                      <div className="flex items-center gap-3">
                        <span className="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/20 flex items-center justify-center text-red-600 text-lg">
                          {activeCategDetails?.randomIcon && renderIcon(activeCategDetails.randomIcon)}
                        </span>
                  ...
                  <SelectContent className="shadow-lg">
                    <div className="mb-2">
                      <SelectItem value="all" className="py-3 hover:bg-red-50 dark:hover:bg-red-900/20">
                        <div className="flex items-center gap-3">
                          <span className="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/20 flex items-center justify-center text-red-600 text-lg">
                    ...
                    {categoriesWithIcons.map((category) => (
                      category.slug2 !== 'all' && (
                        <div className="mb-2" key={category.slug2}>
                          <SelectItem value={category.slug2} className="py-3 hover:bg-red-50 dark:hover:bg-red-900/20">
                            <div className="flex items-center gap-3">
                              <span className="w-8 h-8 rounded-full bg-red-100 dark:bg-red-900/20 flex items-center justify-center text-red-600 text-lg">
              ...
              <motion.div key={activeCategory} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="text-center mb-8 relative shadow-sm rounded-lg">
          ...
          <div className="grid gap-4">
                    ...
                    {isSitesLoading ? (
                      ...
                      <AnimatePresence>
                        {sites
                          ...
                          .map((site, index) => (
                            <motion.div key={site._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }}>
                              <Card id={`card-${site._id}`} className={`p-4 hover:bg-muted/50 transition-colors cursor-pointer ${expandedCard === site._id ? 'bg-muted/50' : ''}`} onClick={(e) => handleCardClick(e, site._id)}>
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-4">
                                    <div className="text-sm font-medium text-muted-foreground">#{site.order}</div>
                                    <div className="w-6 h-6 flex-shrink-0">
                                      {site.favicon_url ? (
                                        <img src={site.favicon_url} alt={`${site.title} favicon`} className="w-full h-full object-contain rounded-full border border-muted/20" onError={handleImageError} />
                                      ) : (
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button variant="outline" size="sm" className="gap-2 transition-transform hover:scale-105" onClick={() => handleVisitSite(site.url)}>Visit Site<ExternalLink className="w-4 h-4" /></Button>
                                    <Button variant="ghost" size="icon" className="h-8 w-8">
                                      <ChevronDown className={`w-4 h-4 transition-transform duration-200 ${expandedCard === site._id ? 'rotate-180' : ''}`} />
                                    </Button>
                                  </div>
                                </div>
                                ...
                                <AnimatePresence>
                                  {expandedCard === site._id && (
                                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.4, ease: 'easeInOut', delay: 0.1 }} className="mt-4 pt-4 border-t">
                                      <p className="text-sm text-muted-foreground">{site.description}</p>
                                      {site.keywords && (
                                        <div className="mt-2 flex flex-wrap gap-2">
                                          {site.keywords.split(',').map((keyword, idx) => (
                                            <span key={idx} className="text-xs px-2 py-1 bg-muted rounded-full text-muted-foreground">{keyword.trim()}</span>
                                          ))}
                                        </div>
                                      )}
                                    </motion.div>
            ...
            {filteredCategories.length === 0 && !isSitesLoading && (
